...
def mc(rho, beta, eps, config):
    ...
    # G = nx.barabasi_albert_graph(N, 20, seed=9876789)
    # A0 = np.asarray(nx.adjacency_matrix(G))/2
    # G = nx.complete_graph(N)
    # A0 = np.asarray(nx.adjacency_matrix(G))/2
    A0 = (np.ones((N,N)) - np.identity(N))/2
    A = A0.copy()
    ...
    def energy(hi, hj):
        ...
        Ep = -Gamma*np.log(eps + (1-2*eps)*erfc(-X/sqrt2)/2)
        return Ep
    ...
    for t in xrange(sweeps*N):
        ...
        j = np.random.choice(N, p=pj)

        # computing the local field for agents i and j
        h = z.dot(w.T)/sqrtD
        hi, hj = h[i], h[j]

        # getting the effective inflence of agent j
        # Yj = alpha*(A[:, j].sum())/N

...
def mc(rho, beta, eps, config):
    ...
    def energy(hi, hj, rij):
        ...
        Ep = (eps + (1-2*eps)*X) + np.sign((eps + (1-2*eps)*X))
        Ei = -a*X + b*np.sign(X)
        return Ep + Ei
    ...
    for t in xrange(sweeps*N):
        ...
        # updating the reputation i gives to j
        rij = A[i,j]
        A[i,j] = rij + rij*(1-rij)*np.sign(hi*hj)*dr
        # amp = (1 - hi*hj/norm/norm)/2
        # A[i,j] = rij + rij*(1-rij)*np.sign(hi*hj)*dr*amp
        ...
        E = energy(nhi, hj, A[i,j])  #  proposed energy
